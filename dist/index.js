...
class TerserPlugin {
  ...
  *generateTasks(compiler, compilation, chunks, processedAssets) {
    const existingAssets = new Set(Object.keys(compilation.assets).map(assetFilename => TerserPlugin.removeQueryString(assetFilename)));
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (commentsFilename && TerserPlugin.hasAsset(commentsFilename, existingAssets)) {
  ...
  apply(compiler) {
    ...
    const optimizeFn = async (compilation, chunks) => {
      ...
      const tasks = this.generateTasks(compiler, compilation, chunks, processedAssets);
